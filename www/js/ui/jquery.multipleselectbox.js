/*!
 * jQuery Multiple Select Box Plugin 0.4.7
 * 
...
( function($) {
	...
	function initializeMultipleSelectBox($container, options) {
		var $rows = $container.getCachedRows();
	 ...
	 * @return jQuery
	 */
	$.fn.getCachedRows = function(isReNew) {
	...
	$.fn.drawMultipleSelectBox = function(startIndex, currentIndex, isGetPositionByCache, isSelectionSaved, isScrollBarFrozen) {
		return this.each( function() {
			...
			var $startRow = $rows.eq(startIndex);
			if (startIndex < 0 || currentIndex < 0 || options.maxLimit == 0 || (startIndex == containerInfo.lastStartIndex && currentIndex == containerInfo.lastCurrentIndex) || $startRow.hasClass(PLUGIN_STYLE_DISABLED)) {
				return this;
			}

			var selectedCount = 0;
			var stateArray = $.map($rows, function(row, index) {
				var $childRow = $(row);
				/* clear and group by style */
				$childRow.removeClass(PLUGIN_STYLE_SELECTING);
				if ($childRow.hasClass(PLUGIN_STYLE_DISABLED)) {
					return -1;
				} else if ($childRow.hasClass(PLUGIN_STYLE_SELECTED)) {
					if (isSelectionSaved) {
						selectedCount++;
						return 1;
					}
			...
			for ( var i = Math.min(startIndex, currentIndex); i <= Math.max(startIndex, currentIndex); i++) {
				if (stateArray[i] != -1) {
					$rows.eq(i).addClass(PLUGIN_STYLE_SELECTED);
				}
			}
	...
	$.fn.serializeMultipleSelectBox = function() {
		...
		this.each( function() {
			var $container = $(this);
			var options = $container.data("options");
			resultList.push($.map($container.getCachedRows(), function(row, index) {
				var $childRow = $(row);
				var resultValue = null;
				if (!$childRow.hasClass(PLUGIN_STYLE_DISABLED) && $childRow.hasClass(PLUGIN_STYLE_SELECTED) && (resultValue = options.valueRendererArray[index]) == null) {
	...
	$.fn.recalculateMultipleSelectBox = function(isResetContainerInfo, isResetRowsInfo, isResetHistory, isResetRowCache) {
		return this.each( function() {
			var $container = $(this);
			var $rows = $container.getCachedRows(isResetRowCache);
			var containerInfo = $container.data("info");
			if (!containerInfo) {
			...
			if (isResetRowsInfo) {
				...
				$rows.each( function(i) {
					var $childRow = $(this);
					var childRowTopPos = this.offsetTop;
					if (i == 0) {
						firstTopPos = childRowTopPos;
					}
					childRowTopPos -= firstTopPos;

					$childRow.data("index", i);
					rowInfoArray.push({
			...
			if (isResetContainerInfo) {
				containerInfo.topPos = this.offsetTop;
